...
public class CursoDAO {
	...
	public Curso get(int id) {
		...
		try {
			...
			rs.next();
			curso = new Curso(rs.getInt("id"), rs.getString("idioma"), rs.getString("autor"), rs.getString("conteudo"), rs.getString("url"), rs.getString("titulo"), rs.getInt("horas"), rs.getInt("avaliacao"), rs.getInt("areaid"));			st.close();
		} catch (Exception e) {
			System.err.println(e.getMessage());
		}
		return curso;
...
//	}
	...
	public void remove(Curso c) {
		try {  
			Statement st = conexao.createStatement();
			st.executeUpdate("DELETE FROM curso WHERE id = " + c.getId());
			st.close();
		} catch (SQLException u) {  
			throw new RuntimeException(u);
		}
